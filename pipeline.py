...
def merge_fastq_pairs(fastqs, paths, i=1):
	print('Merging reads... ')
	cmd_merge = os.system(''
	+ 'cat ' + ' '.join(fastqs['f']) + ' > ' + paths['out'] + '/merge/' + str(i) + '.raw.r1.fastq && '
	+ 'cat ' + ' '.join(fastqs['r']) + ' > ' + paths['out'] + '/merge/' + str(i) + '.raw.r2.fastq && '
	+ 'interleave-reads.py ' + paths['out'] + '/merge/' + str(i) + '.raw.r1.fastq ' + paths['out']
	+ '/merge/' + str(i) + '.raw.r2.fastq 2> /dev/null > ' + paths['out'] + '/merge/' + str(i) + '.raw.r12.fastq')
...
def sample_reads(n_reads, paths, i=1):
	...
	+ 'cat ' + paths['out'] + '/merge/' + str(i) + '.raw.r12.fastq | seqtk sample - ' + str(n_reads_sample)
	+ ' | seqtk seq -a - > ' + paths['out'] + '/sample/' + str(i) + '.sample.fasta')
...
def map_reads(reference_fa_path, paths, threads, i=1):
	...
	cmds_align = [
		...
		+ ' --threads ' + str(threads) + ' -A 80 > ' + paths['out'] + '/map/' + str(i) + '.segemehl_mapped.sam',
		'samtools view -bS ' + paths['out'] + '/map/' + str(i) + '.segemehl_mapped.sam | samtools sort - '
		+ paths['out'] + '/map/' + str(i) + '.segemehl_mapped; ' + 'samtools index ' + paths['out']
		+ '/map/' + str(i) + '.segemehl_mapped.bam',
		'samtools mpileup -d 1000 -f ' + reference_fa_path + ' ' + paths['out']
		+ '/map/' + str(i) + '.segemehl_mapped.bam > ' + paths['out'] + '/map/' + str(i) + '.segemehl_mapped.pile',
		'samtools mpileup -ud 1000 -f ' + reference_fa_path + ' ' + paths['out'] + '/map/' + str(i) + '.segemehl_mapped.bam | '
		+ 'bcftools call -c | vcfutils.pl vcf2fq | seqtk seq -a - | fasta_formatter '
		+ '-o ' + paths['out'] + '/map/' + str(i) + '.consensus.fasta']
...
def normalise(norm_k_list, norm_c_list, paths, i=1):
	...
	for norm_perm in norm_perms:
		...
		cmd_norm = os.system(''
		+ 'normalize-by-median.py -C ' + c + ' -k ' + k + ' -N 1 -x 1e9 -p ' + paths['out'] + '/trim/' + str(i) + '.trim.r12_pe.fastq '
		+ '-o ' + paths['out'] + '/norm/' + str(i) + '.norm_k' + k + 'c' + c + '.r12_pe.fastq &> /dev/null && '
		+ 'normalize-by-median.py -C ' + c + ' -k ' + k + ' -N 1 -x 1e9 ' + paths['out'] + '/trim/' + str(i) + '.trim.se.fastq '
		+ '-o ' + paths['out'] + '/norm/' + str(i) + '.norm_k' + k + 'c' + c + '.se.fastq &> /dev/null && '
		+ paths['pipe'] + '/res/fastq_deinterleave ' + paths['out'] + '/norm/' + str(i) + '.norm_k' + k + 'c' + c + '.r12_pe.fastq '
		+ paths['out'] + '/norm/' + str(i) + '.norm_k' + k + 'c' + c + '.r1_pe.fastq '
		+ paths['out'] + '/norm/' + str(i) + '.norm_k' + k + 'c' + c + '.r2_pe.fastq &> /dev/null && '
		+ 'cat ' + paths['out'] + '/norm/' + str(i) + '.norm_k' + k + 'c' + c + '.r12_pe.fastq '
		+ paths['out'] + '/norm/' + str(i) + '.norm_k' + k + 'c' + c + '.se.fastq > '
		+ paths['out'] + '/norm/' + str(i) + '.norm_k' + k + 'c' + c + '.pe_and_se.fastq')
